...
// Ito ay pang-capture ng screen gamit ang DirectX.
// Ito ay isesave sa mp4 file.
...
const wchar_t destination_pathname[] = { L"G:\\User\\software\\data\\screenrecorder" };
...
DWORD WINAPI ThreadFunc(LPVOID lpParam)
{
    ...
    LONG stride;
    UINT32 fps_nume, fps_deno, avgbitrate;
    LARGE_INTEGER freq, t1, t2, to;
    ...
    hr = capture.Open(&width, &height, &stride, &buffersize);

    //      YouTube recommended upload encoding settings
    //      https://support.google.com/youtube/answer/1722171?hl=en#zippy=%2Cbitrate

    //      Recommended video bitrates for SDR
    // 
    //      +------------+------------------------------------+--------------------------------+
    ...
    //      Recommended video bitrates for HDR
    // 
    //      +------------+------------------------------------+--------------------------------+
    //      | Type       | Video Bitrate, Standard Frame Rate | Video Bitrate, High Frame Rate |
    //      |            | (24, 25, 30)                       | (48, 50, 60)                   |
    ...
    if (SUCCEEDED(hr)) {
        ...
        hr = file.Open(filename, width, height, stride, fps_nume, fps_deno, avgbitrate);
        ...
        while (true) {
            ...
            //     1 second = 1 second
            //     1 second = 10^9 * 10^-9 second
            //     1 second = 10^9 nanosecond
            //     1 second = 10^7 * 10^2 nanosecond
            //     1 second = 10^7 hundreds of nanosecond
            //     1 second = 10,000,000 hundreds of nanoseconds

            QueryPerformanceCounter(&t2);

            duration = ((t2.QuadPart - t1.QuadPart) * 10000000LL) / freq.QuadPart;
    ...
    QueryPerformanceCounter(&t2);
    duration = ((t2.QuadPart - to.QuadPart) * 1000LL) / freq.QuadPart;
