...
// contructor
CClient::CClient(const char* port)
{
	thread1 = NULL;
	hWnd = NULL;
...
//
void CClient::Run(HWND hWnd, HWND hTree, HWND hList, HWND hEdit, char* ip, wchar_t* name)
{
	DWORD id;
	...
	this->hList = hList;
	this->hEdit = hEdit;
	strcpy_s(this->ip, MAX_IP_STRING, ip);
...
DWORD WINAPI CClient::Function1(LPVOID lpParam)
{
	...
	errcode = getaddrinfo(p->ip, p->port, &hints, &result);
	if (errcode != 0) return 0;

	for (ptr = result; ptr != NULL; ptr = ptr->ai_next) {

		p->socket = ::socket(ptr->ai_family, ptr->ai_socktype, ptr->ai_protocol);
		if (p->socket == INVALID_SOCKET) continue;

		errcode = connect(p->socket, ptr->ai_addr, (int)ptr->ai_addrlen);
		if (errcode == SOCKET_ERROR) continue;
