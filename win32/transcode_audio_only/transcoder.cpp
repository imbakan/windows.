...
// convert video file to audio only file
...
HRESULT CTranscoder::CreateTranscodeProfile(IMFTranscodeProfile** ppProfile)
{
	...
	// Audio Subtype GUIDs
	// https://learn.microsoft.com/en-us/windows/win32/medfound/audio-subtype-guids
	hr = MFTranscodeGetAudioOutputAvailableTypes(MFAudioFormat_AAC, dwFlags, NULL, &pAvailableTypes);
	if (FAILED(hr))
	{
		goto done;
	}
