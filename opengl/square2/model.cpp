...
void CModel::Generate(int vertex_size, int* stride, float** vertices, int* size1, unsigned int** indices, int* size2, int* count)
{
    int k, allocsize;
...
void CModel::Create(GLuint handle)
{
    ...
    int vertex_size, stride, size1, size2, vertex_index;
    ...
    Generate(vertex_size, &stride, &vertices, &size1, &indices, &size2, &count);
