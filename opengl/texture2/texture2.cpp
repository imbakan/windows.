...
void OnOpacity(HWND hWnd, WPARAM wParam, LPARAM lParam);
...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_TEXTURE2, szWindowClass, MAX_LOADSTRING);
    ...
    wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_TEXTURE2));
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
    case WM_OPACITY: OnOpacity(hWnd, wParam, lParam); break;

    case WM_COMMAND:
        switch (LOWORD(wParam))
        {
        case IDM_OPACITY:		OnViewOpacity(hWnd);		break;
        default:
...
void OnRButtonDown(HWND hWnd, WORD Key, int x, int y)
{
    ...
    hMenu = LoadMenu(hInst, MAKEINTRESOURCEW(IDC_TEXTURE2));
...
void OnCreate(HWND hWnd, HDC* hDC)
{
    ...
    *hDC = GetDC(hWnd);                         // get the device context for our window
    ...
    wcscat_s(filename, MAX_PATH, L"\\opengl.png");
    ...
    // Vulkan
    wcscpy_s(filename, MAX_PATH, pathname);
    wcscat_s(filename, MAX_PATH, L"\\vulkan.png");
    ...
    char source2[] = "#version 330\n"
        ...
        "void main()\n"
        "{\n"
        "FragColor = mix(texture(sampler0, st), texture(sampler1, st), opacity);\n"
        "}\n";

    shader.Create(source1, source2);
    shader.Use();
    model.Create(shader.Get(), (float)width, (float)height, 50.0f);
