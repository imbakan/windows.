...
void CTensionDialog::OnInitDialog(HWND hWnd)
{
	// center dialog
	//
	//          +----------------------------+
	...
	swprintf_s(str, 100, L"%0.3f", value);
	SetDlgItemText(hWnd, IDC_TEXT1, str);
}

void CTensionDialog::OnHScroll(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	...
	if (GetDlgCtrlID(hScrollBar) == IDC_SCROLLBAR1 && valid) {
		...
		swprintf_s(str, 100, L"%6.3f", value);
		SetDlgItemText(hWnd, IDC_TEXT1, str);

		HWND hwnd = GetParent(hWnd);
		SendMessage(hwnd, WM_TENSION, (WPARAM)0, (LPARAM)&value);
	}
}

void CTensionDialog::OnClose(HWND hWnd)
{
	DestroyWindow(hWnd);
	hDialog = NULL;
}

void CTensionDialog::Show(HWND hWnd, HINSTANCE hInstance, DLGPROC pProc, double value)
{
...
void CTensionDialog::Set(double value)
{
	if (!IsWindow(hDialog)) return;
...
void CTensionDialog::CloseWindow()
{
	if (!IsWindow(hDialog)) return;
	PostMessage(hDialog, WM_CLOSE, 0, 0);
}

INT_PTR CTensionDialog::Proc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
