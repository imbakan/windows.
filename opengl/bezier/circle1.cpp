...
void CCircle1::Generate(int vertex_size, int* stride, float** vertices, int* size, int* count, double radius)
{
    int i, k, n, allocsize;
    double a1, a2, d;
    float x, y, z;
    ...
    *size = (*count) * (*stride);
    ...
    for (i = 0; i < (*count); i++) {
        ...
        x = (float)(radius * cos(a2));
        y = (float)(radius * sin(a2));
...
void CCircle1::Create(GLuint handle, double radius, float* color)
{
...
void CCircle1::Destroy()
{
    glDeleteVertexArrays(1, &vao);
}

void CCircle1::Draw(float* matrix)
{
