...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_CIRCLE1, szWindowClass, MAX_LOADSTRING);
    ...
    wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_CIRCLE1));
...
void OnCreate(HWND hWnd, HDC* hDC)
{
    ...
    *hDC = GetDC(hWnd);                         // get the device context for our window
    ...
    wcscat_s(filename1, MAX_PATH, L"\\vertex.txt");

    wcscpy_s(filename2, MAX_PATH, pathname);
    wcscat_s(filename2, MAX_PATH, L"\\fragment.txt");
